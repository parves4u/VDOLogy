...
const SliderSection: React.FC = () => {
  ...
  const settings = {
    ...
    nextArrow: <NextArrow />,
    prevArrow: <PrevArrow />,
    beforeChange: () => setActiveVideoId(null) // Reset active video when changing slides
  };
  ...
  return (
    <div className="relative max-w-4xl mx-auto px-4 lg:px-0 mt-20 mb-20">
      <h1 className="px-5 py-2 bg-white/10 border border-white/20 text-white font-medium rounded-full w-fit mx-auto" id="RecentProjects">
        Recent Projects
      </h1>
      ...
      <Slider {...settings} className="variable-width">
        {slides.map((slide) => (
          <div className="px-4 h-80" key={slide.id}>
            <div
            ...
            >
              <img
                ...
                alt={slide.alt}
                loading="lazy"
                className={`rounded-lg size-full object-cover cursor-pointer ${activeVideoId === slide.id ? 'opacity-0' : 'opacity-100'}`}
              />
